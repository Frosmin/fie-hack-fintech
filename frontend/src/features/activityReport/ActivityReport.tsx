...
function groupByDate(txs: Transaction[]) {
  ...
  txs.forEach((tx) => {
    const key = new Date(tx.date).toLocaleDateString("es-BO", { day: "2-digit", month: "short" });
...
export function ActivityReport() {
  ...
  const totalIncome = useMemo(
    () => transactions.filter((t) => t.type === "DEPOSIT" || t.type === "REFUND").reduce((s, t) => s + parseFloat(t.amount), 0),
    [transactions]
  );
  const totalExpense = useMemo(
    () => transactions.filter((t) => t.type === "PAYMENT" || t.type === "WITHDRAWAL").reduce((s, t) => s + parseFloat(t.amount), 0),
    [transactions]
  );
  ...
  const donutGradient = useMemo(() => {
    if (typeDistribution.entries.length === 0) return "conic-gradient(#e5ebf6 0deg 360deg)";
    let acc = 0;
  ...
  if (error && !activity) {
    return (
      <section className="ar">
        ...
        <button className="ar__back" onClick={() => navigate(-1)}>← Volver</button>
      </section>
    );
  }
    ...
    <section className="ar">
      ...
      <div className="ar__summary-grid ar__stagger-3">
        ...
        <div className="ar__summary-card ar__summary-card--income">
          ...
          <div className="ar__summary-card-value">{formatMoney(totalIncome)}</div>
          <div className="ar__summary-card-sub">
            <span className="ar__summary-card-trend is-up">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                <path d="M12 19V5M5 12l7-7 7 7" />
              </svg>
              {transactions.filter((t) => t.type === "DEPOSIT" || t.type === "REFUND").length} depósitos
            </span>
          </div>
        </div>
        ...
        <div className="ar__summary-card ar__summary-card--expense">
          ...
          <div className="ar__summary-card-value">{formatMoney(totalExpense)}</div>
          <div className="ar__summary-card-sub">
            <span className="ar__summary-card-trend is-down">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                <path d="M12 5v14M19 12l-7 7-7-7" />
              </svg>
              {transactions.filter((t) => t.type === "PAYMENT" || t.type === "WITHDRAWAL").length} pagos
            </span>
          </div>
        </div>
      ...
      <div className="ar__charts-grid ar__stagger-4">
        ...
        <div className="ar__chart-card">
          <div className="ar__chart-card-header">
            ...
            <span className="ar__chart-card-badge">Últimos {barData.length} períodos</span>
          </div>
          {barData.length > 0 ? (
            ...
            <div style={{ textAlign: "center", color: "var(--muted)", padding: "40px 0", fontSize: ".88rem" }}>
        ...
        <div className="ar__chart-card">
          <div className="ar__chart-card-header">
            ...
            <span className="ar__chart-card-badge">{typeDistribution.entries.length} tipos</span>
          </div>
            ...
            <div className="ar__donut">
              <div
                ...
                style={{
                  ...
                  WebkitMask: "radial-gradient(circle, transparent 55%, black 55.5%)",
                }}
              />
              <div className="ar__donut-center">
                <span className="ar__donut-center-value">{formatMoney(typeDistribution.total)}</span>
            ...
            <div className="ar__donut-legend">
              {typeDistribution.entries.map((e) => (
                <div key={e.type} className="ar__donut-legend-item">
                  <div className="ar__donut-legend-color" style={{ background: e.color }} />
                  <div className="ar__donut-legend-info">
                    <span className="ar__donut-legend-name">{TYPE_LABELS[e.type]}</span>
      ...
      <div className="ar__filters ar__stagger-5">
        ...
        {(["ALL", "DEPOSIT", "WITHDRAWAL", "TRANSFER", "PAYMENT", "REFUND"] as FilterType[]).map(
          (type) => (
            <button
              key={type}
                  ...
                  <tr key={tx.id}>
                    <td>
                      <div className="ar__table-tx-info">
                        <div className={`ar__table-tx-indicator ${isIncome ? "is-income" : "is-expense"}`}>
                          {TYPE_ICONS[tx.type] || "•"}
                        </div>
                        <div className="ar__table-tx-details">
                          <span className="ar__table-tx-name">{tx.nameCuate}</span>
                          {tx.description && (
                            <span className="ar__table-tx-desc">{tx.description}</span>
                          )}
                        </div>
                      </div>
                    </td>
                    <td>
                      <span className={`ar__table-type-badge ar__table-type-badge--${tx.type}`}>
                        {TYPE_LABELS[tx.type]}
                      </span>
                    </td>
                    <td>
                      <span className="ar__table-date">{formatShortDate(tx.date)}</span>
                    </td>
                    <td>
                      <span className={`ar__table-status ar__table-status--${tx.status.toLowerCase()}`}>
                        {STATUS_LABELS[tx.status]}
                      </span>
                    </td>
                    <td>
                      <span className={`ar__table-amount ${isIncome ? "is-income" : "is-expense"}`}>
